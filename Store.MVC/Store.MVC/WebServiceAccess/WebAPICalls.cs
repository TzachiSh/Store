...
namespace Store.MVC.WebServiceAccess
{
    public class WebApiCalls : WebApiCallsBase, IWebApiCalls
    {
        ...
        public async Task<int> PurchaseCartAsync(Customer customer)
        {
            ...
            //{ "Id":1,"FullName":"Super Store","EmailAddress":"Store@secrets.com"}
            //  http://localhost:40001/api/shoppingcart/0/buy 
            var json = JsonConvert.SerializeObject(customer);
            var uri = $"{CartBaseUri}{customer.Id}/buy";
            return int.Parse(await PostRequestAsync(uri, json));
        }
        ...
        public async Task<string> UpdateCartItemAsync(ShoppingCartRecord item)
        {
            ...
            return await PutRequestAsync(uri, json);
        }
  ...
  int customerId, int orderId)
        ...
        public async Task<IList<ProductAndCategoryBase>> GetProductsAsync()
        {
            return await GetItemListAsync<ProductAndCategoryBase>($"{ProductBaseUri}");
        }

        public async Task<CookieViewModel> LoginAsync(LoginViewModel loginViewModel)
        {
            var uri = $"{ServiceAddress}api/account/login";
            var json = JsonConvert.SerializeObject(loginViewModel);

            var result = await PostRequestAsync(uri, json);

            return JsonConvert.DeserializeObject<CookieViewModel>(result);
        }

        public async Task<string> RegisterAsync(RegisterViewModel RegisterViewModel)
        {
            var uri = $"{ServiceAddress}api/account/Register";
